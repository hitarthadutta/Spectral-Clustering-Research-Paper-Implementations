...
def constrained_spectral_clustering(X, Q, beta, n_clusters=3):
    ...
    positive_indices = np.where(eigvals > 1e-8)[0]
    if len(positive_indices) == 0:
        warnings.warn("No positive eigenvalues found. Falling back to unconstrained clustering.")
        sc = SpectralClustering(n_clusters=n_clusters, affinity='rbf', random_state=42)
        return sc.fit_predict(X)
...
if __name__ == "__main__":
    ...
    print("Running comparison of CSP, ModAff, and GrBias on Iris dataset...")
    for p in percentages:
        ...
        for method in results.keys():
            mean_ri = np.mean(trial_results[method])
            std_ri = np.std(trial_results[method])
            results[method].append((mean_ri, std_ri))

        print(f"Percentage of Known Labels: {p*100:.0f}% -> "
              f"CSP: {results['csp'][-1][0]:.3f}, "
              f"ModAff: {results['modaff'][-1][0]:.3f}, "
              f"GrBias: {results['grbias'][-1][0]:.3f}")
    ...
    plt.plot(x_axis, mean_csp, marker='o', linestyle='-', label='CSP (Proposed)')
    plt.fill_between(x_axis, mean_csp - std_csp, mean_csp + std_csp, alpha=0.2)
    ...
    plt.title('Comparison of Constrained Spectral Clustering Methods on Iris Dataset', fontsize=16)
